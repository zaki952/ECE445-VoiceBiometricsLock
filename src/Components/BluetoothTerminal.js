...
export class BluetoothTerminal {
   ...
   */
   ...
   read() {
    ...
    return this._readFromCharacteristic(this._characteristicRx);
   }
  receive(data) {
  ...
  send(data) {
    ...
    let promise = this._writeToCharacteristic(this._characteristicTx, chunks[0]);
    ...
    for (let i = 1; i < chunks.length; i++) {
      ...
      promise = promise.then(() => new Promise((resolve, reject) => {
        ...
        this._writeToCharacteristic(this._characteristicTx, chunks[i]).
            then(resolve).
            catch(reject);
      }));
    }
  ...
  _connectToDevice(device) {
    return (device ? Promise.resolve(device) : this._requestBluetoothDevice()).
        then((device) => this._connectDeviceAndCacheCharacteristic(device)).
           then((characteristics) => this._startNotifications(characteristics[1])).
        catch((error) => {
          this._log(error);
  ...
  _readFromCharacteristic(characteristic) {
    let ret = characteristic.readValue();
    return ret;
  }
